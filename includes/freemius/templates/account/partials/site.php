					...
					<?php if ( ! is_object( $main_license ) || $main_license->id != $license->id ) : ?>
						...
						<?php if ( ! $license->is_lifetime() && is_object( $subscription ) ) : ?>
                            ...
                            <tr <?php if ( 1 == $row_index % 2 ) {
                                ...
                                <td>
                                    <nobr><?php fs_esc_html_echo_inline( 'Subscription', 'subscription', $slug ) ?>:
                                    </nobr>
                                </td>
								<?php
                                ...
                                <td><?php if ( $is_active_subscription ) : ?>
                                        ...
                                        <form id="fs_downgrade"
                                              action="<?php echo $fs->_get_admin_page_url( 'account' ) ?>"
                                              method="POST">
                                            <input type="hidden" name="fs_action" value="<?php echo $action ?>">
											<?php wp_nonce_field( trim( "{$action}:{$blog_id}", ':' ) ) ?>
                                            <input type="hidden" name="blog_id" value="<?php echo $blog_id ?>">
                                            <button class="button button-small"
                                                    onclick="if (confirm('<?php echo esc_attr( $downgrade_confirmation_message . ' ' . $after_downgrade_message . ' ' . $prices_increase_text ) ?>')) { this.parentNode.submit(); } else { return false; }"><?php echo $downgrade_text ?></button>
                                        </form>
									<?php endif ?></td>
                            </tr>
							<?php $row_index ++ ?>
