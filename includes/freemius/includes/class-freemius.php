...
------------------------------------------------------------------------------------------------------------------*/
			...
			if ( $this->is_plugin() ) {
				if ( self::is_plugin_install_page() && true !== fs_request_get_bool( 'fs_allow_updater_and_dialog' ) ) {
					...
					add_filter( 'site_transient_update_plugins', array(
						'Freemius',
						'_remove_fs_updates_from_plugin_install_page'
					), 10, 2 );
	...
	private static function _load_required_static() {
		...
		if ( self::is_plugins_page() || self::is_themes_page() ) {
			 ...
			 * @since 2.3.0
			 */
			add_action( 'admin_footer-' . self::get_current_page(), array(
				'Freemius',
				'_maybe_add_beta_label_to_plugins_and_handle_confirmation'
			) );
		}
...
//			}
	...
	function _add_tracking_links() {
		...
		if ( $this->add_ajax_action( 'update_tracking_permission', array(
			&$this,
			'_update_tracking_permission_callback'
		) ) ) {
			return;
		}
